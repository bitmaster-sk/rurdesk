...
async function login(page) {
    ...
    const [res] = await Promise.all([
        page.waitForResponse((r) => r.url().includes("/api/public/login"), {
            timeout: 30000,
        }),
    ...
    if (!res.ok())
        throw new Error(
            `login failed: POST /api/public/login → ${res.status()} (check CAPTURE_EMAIL/PASSWORD, stack up?)`,
        );
...
async function shoot(page, name) {
    if (onLoginPage(page))
        throw new Error("on /login — session not authenticated");
    ...
    return {
        ...
        async moveTo(page, x, y, steps = 40) {
            ...
            for (let i = 1; i <= steps; i++) {
                ...
                await page.evaluate(
                    ([px, py]) => window.__setCur && window.__setCur(px, py),
                    [nx, ny],
                );
...
function parseIssueRef(input) {
    ...
    if (m) return m[1];
    return (
        s.split("?")[0].split("#")[0].replace(/\/+$/, "").split("/").pop() || s
    );
}
...
async function shootQuality(page, file, idIssue) {
    ...
    if (collapsed) {
        await page.locator(".quality-panel__header").first().click();
        await page.waitForTimeout(800);
    }
...
const SHOTS = [
    ...
    ...[
        ...
        desc: `Issue ${v} view`,
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/${v}`);
            await settle(page);
    ...
    // Day|Day|Week|Month and Comfortable|Compact); exact-text avoids "Quarter Day"
    {
        ...
        file: "view-gantt.png",
        kind: "png",
        desc: "Issue gantt view (Day zoom, Compact density)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/gantt`);
            await settle(page);
            for (const label of ["Day", "Compact"]) {
                const btn = page.getByText(label, { exact: true }).first();
                if (await btn.count()) {
            ...
            await settle(page);
            await shoot(page, "view-gantt.png");
        },
    },
    ...
    // kanban is captured on the Swimlane layout (toolbar Columns | Swimlane)
    {
        ...
        kind: "png",
        desc: "Issue kanban view (Swimlane layout)",
        run: async (page) => {
...

    {
        ...
        file: "hero.png",
        kind: "png",
        desc: "Hero (gantt on Day zoom, Compact density)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/gantt`);
            await settle(page);
            for (const label of ["Day", "Compact"]) {
...

    {
        ...
        kind: "png",
        desc: "Project overview dashboard",
        run: async (page) => {
...

    {
        ...
        kind: "png",
        desc: "Issue detail (incl. participants card)",
        run: async (page) => {
            if (ISSUE) {
                await gotoApp(
                    page,
                    `${BASE}/project/${PROJECT}/issue/${ISSUE}`,
                );
            } else {
                await gotoApp(
                    page,
                    `${BASE}/project/${PROJECT}/issue/view/table`,
                );
                await settle(page);
                const row = page
                    .locator("tbody tr a, .p-datatable-tbody tr")
                    .first();
    ...
    {
        id: "quality-high",
        file: "quality-high.png",
        kind: "png",
        needsIssue: true,
        envIssue: "CAPTURE_QUALITY_HIGH_ISSUE",
        desc: "Task-quality panel, high score (needs an issue id / URL)",
        run: async (page, _ctx, shot) =>
            shootQuality(page, "quality-high.png", shot._issueId),
    },
    {
        id: "quality-low",
        file: "quality-low.png",
        kind: "png",
        needsIssue: true,
        envIssue: "CAPTURE_QUALITY_LOW_ISSUE",
        desc: "Task-quality panel, low score + suggestions (needs an issue id / URL)",
        run: async (page, _ctx, shot) =>
            shootQuality(page, "quality-low.png", shot._issueId),
    },
    ...
    // MUTATES issue #DEMO_ISSUE (assignee + a comment).
    {
        ...
        kind: "png",
        desc: "Participants card expanded, 2 participants — MUTATES (assign + comment)",
        run: async (page) => {
            await gotoApp(
                page,
                `${BASE}/project/${PROJECT}/issue/${DEMO_ISSUE}`,
            );
            await settle(page);
            // assign to the 2nd human user via the assignee dropdown (avoid bots)
            await page.locator("app-user-dropdown").first().click();
            await page.waitForTimeout(500);
            await page.getByText(USER2_NAME, { exact: true }).first().click();
            ...
            await editor.click();
            await editor.type(
                "Taking a look — pairing with " + USER2_NAME + " on this.",
            );
            ...
            await root.screenshot({
                path: path.join(IMG, "participants-panel.png"),
            });
        },
    },
    ...
    // picker. Read-only (nothing posted).
    {
        ...
        kind: "png",
        desc: "Mention picker autocomplete in the comment composer",
        run: async (page) => {
            await gotoApp(
                page,
                `${BASE}/project/${PROJECT}/issue/${DEMO_ISSUE}`,
            ...
            await page.screenshot({
                path: path.join(IMG, "mention-picker.png"),
    ...
    // Posted comment containing a mention → rendered as a chip. MUTATES (posts a comment).
    {
        ...
        kind: "png",
        desc: "Rendered mention chip in a posted comment — MUTATES (posts a comment)",
        run: async (page) => {
            await gotoApp(
                page,
                `${BASE}/project/${PROJECT}/issue/${DEMO_ISSUE}`,
    ...
    // MUTATES issue #DEMO_ISSUE (two time entries).
    {
        ...
        file: "tracker.png",
        kind: "png",
        desc: "Feed filtered to Time chip: two users' tracked time — MUTATES (logs time)",
        run: async (page) => {
    ...
    {
        id: "split-dialog-before-generate",
        file: "split-dialog-before-generate.png",
        kind: "png",
        desc: "Split dialog before Generate (hint step)",
        run: async (page) => {
            await openSplitDialog(
                page,
                SPLIT_ISSUE,
                "separate parsing, formatting and error handling",
            );
            await page
                .locator(".ui-dialog")
                .first()
                .screenshot({
                    path: path.join(IMG, "split-dialog-before-generate.png"),
                });
            await page
    ...
    {
        id: "split-dialog-after-generate",
        file: "split-dialog-after-generate.png",
        kind: "png",
        desc: "Split dialog after Generate (AI children) — calls AI, not saved",
        run: async (page) => {
            await openSplitDialog(
                page,
                SPLIT_ISSUE,
            ...
            if (start !== 0)
                throw new Error(
                    "AI split failed or timed out (no child tasks) — try again",
                );
            ...
            await page
                ...
                .first()
                .screenshot({
                    path: path.join(IMG, "split-dialog-after-generate.png"),
                });
    ...
    // agent-workflow.webm clip still needs live recording (not scriptable here). ===
    {
        ...
        kind: "png",
        needsIssue: true,
        envIssue: "CAPTURE_RUN_ISSUE",
        promptLabel: "run issue",
        desc: "Brainstorming stage comment (needs a run issue id / URL)",
        run: async (page, _c, shot) => {
            await gotoApp(
                page,
                `${BASE}/project/${PROJECT}/issue/${shot._issueId}`,
            );
            await settle(page);
            let el = page
            ...
            await page.waitForTimeout(400);
            await el.screenshot({
                path: path.join(IMG, "stage-brainstorming.png"),
            });
        },
    },
    {
    ...
    },
    {
        ...
        envIssue: "CAPTURE_RUN_ISSUE",
        promptLabel: "run issue",
        desc: "Implementation plan stage comment (diffs)",
        run: async (page, _c, shot) =>
            shootSel(
                page,
                "stage-plan.png",
    ...
    },
    {
        ...
        envIssue: "CAPTURE_RUN_ISSUE",
        promptLabel: "run issue",
        desc: "PR-pushed comment with the PR link (pr_open)",
        run: async (page, _c, shot) =>
            shootSel(
                page,
                "run-pr.png",
    ...
    },
    {
        ...
        envIssue: "CAPTURE_RUN_ISSUE",
        promptLabel: "run issue",
        desc: "Design comment mockup cards (one selected) + surroundings",
        run: async (page, _c, shot) =>
            shootSel(
                page,
                "message-mockup.png",
    ...
    },
    {
        ...
        run: async (page, _c, shot) => {
            ...
            await modal.screenshot({
                path: path.join(IMG, "stage-mockup-preview.png"),
            });
    ...
    // there is no /admin/teams route. Element-screenshot just the teams panel.
    {
        ...
        kind: "png",
        desc: "Admin → teams panel, first team selected (shows its members)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/admin/users`);
            await settle(page);
    ...
    // Cancelled — no user is created
    {
        ...
        kind: "png",
        desc: "New user dialog, filled for a human (not saved)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/admin/users`);
            await settle(page);
            await page
            ...
            await dialog.screenshot({
                path: path.join(IMG, "create-user-dialog.png"),
            });
            await page
                .getByRole("button", { name: "Cancel" })
                .first()
    ...
    // the qwen goose gateway. Cancelled — no bot is created.
    {
        ...
        kind: "png",
        desc: "New user dialog, Bot toggle on, qwen gateway (not saved)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/admin/users`);
            await settle(page);
            await page
            ...
            const dialog = page.locator(".ui-dialog").first();
            await dialog.waitFor({ state: "visible", timeout: 8000 });
            await page.locator("#cu-isbot").check();
            await page.waitForTimeout(400); // reveal bot fields
            await page.fill("#cu-name", "Qwen");
            await page.fill(
                "#cu-gateway-url",
                "http://gateway-goose-qwen-cloud:9090",
            );
            await page.waitForTimeout(400);
            await dialog.screenshot({
                path: path.join(IMG, "agent-create.png"),
            });
            await page
                .getByRole("button", { name: "Cancel" })
                .first()
    ...
    // screenshots that dialog, then DELETES the temp bot again (cleanup).
    {
        ...
        file: "agent-keys.png",
        kind: "png",
        desc: "New bot API-keys dialog — creates a TEMP bot, shoots, then deletes it",
        run: async (page) => {
            ...
            await page
                .locator(".ui-dialog")
                .first()
                .screenshot({ path: path.join(IMG, "agent-keys.png") });
            await page
                .getByRole("button", { name: "Close", exact: true })
                .first()
                .click()
            ...
            try {
                ...
                await row
                    .locator("button:has(.tabler-icon-trash)")
                    .first()
                    .click();
                await page
                ...
                console.warn(
                    `    ! cleanup failed — delete "${BOT_NAME}" manually (${String(e.message).split("\n")[0]})`,
                );
            }
        },
    },
    ...
    // and the TEAMS subblock (further down) are visible without manual scrolling
    {
        ...
        kind: "png",
        desc: "Project settings → members (users + teams)",
        run: async (page) => {
            ...
            await comp.screenshot({
                path: path.join(IMG, "project-members.png"),
            });
        },
    },

    {
        ...
        kind: "png",
        desc: "Project settings → git integrations (list)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/settings`);
    ...
    // Cancel WITHOUT saving (nothing is created)
    {
        ...
        kind: "png",
        desc: "Git integration create form, filled then cancelled (not saved)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/settings`);
            await settle(page);
            const newBtn = page
            ...
            await page.waitForTimeout(500);
            await dialog.screenshot({
                path: path.join(IMG, "git-integration-form.png"),
            });
            await page
                .getByRole("button", { name: "Cancel" })
                .first()
    ...
    // Severity / Assign actions for that task are shown ---
    {
        ...
        kind: "png",
        desc: "Command palette in > (commands) mode over a highlighted task — State/Severity/Assign",
        run: async (page) => {
            ...
            await page
                ...
                .first()
                .click()
                .catch(() => {}); // focus off inputs
            await page.keyboard.press("ArrowDown");
            await page.waitForTimeout(300); // highlight 1st row
            await page.keyboard.press("ArrowDown");
            await page.waitForTimeout(400); // → 2nd row (sets ctx.issue)
    ...
    // the palette. MUTATES the 2nd task's severity + assignee (reversible). ---
    {
        ...
        kind: "webm",
        desc: "Palette flow clip: nav + change severity + assignee of the 2nd task — MUTATES it",
        run: async (page, { cursor, mark }) => {
            await gotoApp(
                page,
                `${BASE}/project/${PROJECT}/issue/view/calendar`,
            );
            await settle(page);
            await cursor.ensure(page);
            mark(); // clip starts on the calendar (login trimmed off)
            await page.waitForTimeout(700);
    ...
    // scope chip" is stale — the lanes are always present; there is no toggle. ---
    {
        ...
        kind: "png",
        desc: "Board with sprint tab strip (cycle + Backlog lanes)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/kanban`);
            await settle(page);
            await selectSwimlane(page);
...

    {
        ...
        kind: "png",
        desc: "Charts band above the board (burndown + velocity)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/kanban`);
            await settle(page);
            await selectSwimlane(page);
            const toggle = page
                .locator('[data-testid="kanban-charts-toggle"] button')
                .first();
            ...
            if (
                !(await page
                    .locator('[data-testid="sprint-charts-band"]')
                    .count())
            ) {
                await toggle.click();
            }
    ...
    // closes it WITHOUT submitting, so no sprint is created. ---
    {
        ...
        kind: "png",
        desc: "Sprint create dialog (opened from ＋ Sprint, not submitted)",
        run: async (page) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/kanban`);
            await settle(page);
            await selectSwimlane(page);
    ...
    // show the new order persisted (gantt_rank). MUTATES row order (reversible). ---
    {
        ...
        kind: "webm",
        desc: "Drag a WBS Scheduled row to reorder (Day+Compact, persisted) — MUTATES order",
        run: async (page, { cursor, mark }) => {
            await gotoApp(page, `${BASE}/project/${PROJECT}/issue/view/gantt`);
            await settle(page);
            ...
            if (await sched.count()) {
                const [sx, sy] = await centerOf(sched);
            ...
            if (n < 3)
                throw new Error(
                    `need ≥3 scheduled rows to show a reorder (found ${n})`,
                );
            ...
            await cursor.dragTo(page, srcX, targetY, 30);
            await page.waitForTimeout(900);
    ...
    // so it needs an EMPTY project (env CAPTURE_KICKSTART_PROJECT / CLI / prompt).
    {
        ...
        kind: "webm",
        needsProject: true,
        envProject: "CAPTURE_KICKSTART_PROJECT",
        promptLabel: "empty project",
        ...
        run: async (page, { cursor, mark }, shot) => {
            ...
            await gotoApp(page, `${BASE}/project/${P}/project-builder`);
            await settle(page);
            await cursor.ensure(page);
            mark(); // clip starts on the builder (login trimmed)
            ...
            await pickDropdownOption(
                page,
                page.locator("app-state-dropdown").first(),
                "New",
            );
            await pickDropdownOption(
                page,
                page.locator("app-severity-dropdown").first(),
                "Low",
            );
            await page.waitForTimeout(400);
            ...
            await page
                ...
                .catch(() => {
                    throw new Error(
                        "kickstart AI did not return a backlog (it errored or timed out server-side — the kickstart AI is flaky; just re-run)",
                    );
                });
            ...
            if (!/\/issue\/view/.test(page.url())) {
                await gotoApp(page, `${BASE}/project/${P}/issue/view/table`);
            }
            await settle(page);
            await page.waitForTimeout(1500);
        },
...
async function runWebmShot(shot) {
    ...
    const newest = webms.sort(
        (a, b) => fs.statSync(b).mtimeMs - fs.statSync(a).mtimeMs,
    )[0];
    const dst = path.join(IMG, shot.file);
    if (newest !== dst) {
        fs.renameSync(newest, dst);
    }
    // trim the login/setup lead-in (start ~0.4s before the marked moment)
    if (markMs !== null) {
        const offset = Math.max(0, (markMs - t0) / 1000 - 0.4);
        try {
            if (trimWebm(dst, offset))
                console.log(`  ✂  trimmed lead-in (${offset.toFixed(1)}s)`);
        } catch (e) {
            console.warn(
                `  ! trim failed (${String(e.message).split("\n")[0]}) — keeping full clip`,
            );
        }
    }
...
async function tryRun(shot, page, ctx) {
    try {
        await shot.run(page, ctx, shot);
        if (shot.kind === "png") console.log(`  ✓ ${shot.file}`);
    } catch (err) {
        console.warn(`  ✗ ${shot.file}: ${String(err.message).split("\n")[0]}`);
    }
}
...
async function resolveIssue(shot, issueMap) {
    let ref =
        process.env[shot.envIssue] ||
        issueMap[shot.id] ||
        issueRefCache[shot.envIssue];
    if (!ref && process.stdin.isTTY)
        ref = await promptLine(
            `  ${shot.promptLabel || shot.id}: paste the issue URL or public id → `,
        );
    if (!ref) {
        console.warn(
            `  ✗ ${shot.file}: no issue id (set ${shot.envIssue}, pass ${shot.id}=<id|url>, or run interactively)`,
        );
        return false;
    }
...
async function resolveProject(shot, issueMap) {
    let ref =
        process.env[shot.envProject] ||
        issueMap[shot.id] ||
        projectRefCache[shot.envProject];
    if (!ref && process.stdin.isTTY)
        ref = await promptLine(
            `  ${shot.promptLabel || shot.id}: enter the project id (empty project — a full backlog will be generated) → `,
        );
    if (!ref) {
        console.warn(
            `  ✗ ${shot.file}: no project id (set ${shot.envProject}, pass ${shot.id}=<id>, or run interactively)`,
        );
        return false;
    }
    ...
    return new Promise((resolve) => {
        ...
        const render = (first = false) => {
            ...
            process.stdout.write(
                "  Pick captures  (↑/↓ move · space toggle · a all · enter run · q quit)\n\n",
            );
            items.forEach((it, i) => {
        ...
        const onKey = (str, key) => {
            ...
            if (key.name === "up")
                ...
                cleanup();
                return resolve([...checked].map((i) => items[i].id));
...
if (argv.includes("--list")) {
    for (const s of SHOTS)
        console.log(
            `${s.kind === "webm" ? "🎬" : "📸"} ${s.id.padEnd(28)} ${s.desc}`,
        );
    ...
    else console.log("nothing picked.");
} else {
    console.log(
        "No TTY and no shot ids given. Use --list, --all, --png, --webm, or pass ids. See header.",
    );
}

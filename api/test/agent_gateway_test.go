...
func (s *AgentGatewaySuite) SetupSuite() {
	s.App = Setup(s.T())
	s.Token = Token(s.T(), s.App)

	s.AgentUserID = s.createAgent("gwbot")
...
func (s *AgentGatewaySuite) gatewayPath(idUser int64) string {
	return fmt.Sprintf("/api/private/admin/user/%d/gateway", idUser)
}

// Assigning a gateway-less agent to an issue must be rejected with 422 so the
// issue is never committed with an assignee that can never dispatch a run.
func (s *AgentGatewaySuite) TestAssignGatewaylessAgentRejects() {
	nogwAgentID := s.createAgent("nogwbot")
	defer s.App.Pool.Exec(context.Background(), "DELETE FROM users.user WHERE id_user = $1", nogwAgentID)

	Request(s.T(), s.App, "POST",
		fmt.Sprintf("/api/private/project/%d/member/user", s.IdProject),
		fmt.Sprintf(`{"idUser":%d,"role":"member"}`, nogwAgentID), s.Token)
	...
	res := Request(s.T(), s.App, "PATCH",
		...
		fmt.Sprintf(`{"title":"gw-test issue","description":"gw-test","assignedTo":%d,"estimated":0}`, nogwAgentID),
		s.Token)
...
func (s *AgentGatewaySuite) TestNonAdminForbidden() {
	path := s.gatewayPath(s.AgentUserID)
...
// Gateways exist only for agent users.
func (s *AgentGatewaySuite) TestGatewayForNonAgentRejected() {
...
func (s *AgentGatewaySuite) TestGatewayLifecycle() {
	...
	var regen model.CreateAgentGatewayRes
...
func TestAgentGatewaySuite(t *testing.T) {
	suite.Run(t, new(AgentGatewaySuite))
}

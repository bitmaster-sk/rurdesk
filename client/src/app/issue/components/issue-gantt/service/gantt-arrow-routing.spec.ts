...
describe('GanttArrowRouting.route', () => {
    it('routes finish-to-start with room via a drop into the target row', () => {
        const { path } = GanttArrowRouting.route(endpoints());
    ...
    it('keeps the long horizontal of a back-to-back chain on the row boundary', () => {
        ...
        const { path } = GanttArrowRouting.route(endpoints({ targetX: 100 }));
    ...
    it('uses the boundary above the source when the target row is above', () => {
        const { path } = GanttArrowRouting.route(
            endpoints({
                sourceY: 96,
                sourceRowIndex: 1,
                targetY: 32,
    ...
    it('fans out arrows sharing an exit side via lanes', () => {
        const inner = GanttArrowRouting.route(endpoints({ exitLane: 0 }));
        const outer = GanttArrowRouting.route(endpoints({ exitLane: 1 }));
    ...
    it('caps lane offsets at MAX_LANE', () => {
        const capped = GanttArrowRouting.route(endpoints({ exitLane: MAX_LANE + 5 }));
        const max = GanttArrowRouting.route(endpoints({ exitLane: MAX_LANE }));
    ...
    it('detours above the row for same-row relations', () => {
        const { path, midY } = GanttArrowRouting.route(
    ...
    it('returns a midpoint lying on the path for the boundary route', () => {
        const { path, midX, midY } = GanttArrowRouting.route(endpoints({ targetX: 100 }));
        const pts = points(path);

...
export abstract class GanttArrowRouting {
    public static route(e: ArrowEndpoints): ArrowRoute {
        ...
        if (e.exitDirection === e.enterDirection) {
            const anchorX =
                e.exitDirection === 'left'
                    ? Math.min(x1, x2) - exitOff
                    : Math.max(x1, x2) + exitOff;

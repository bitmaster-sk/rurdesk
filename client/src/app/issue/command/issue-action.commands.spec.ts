...
describe('IssueActionCommands.build', () => {
    it('returns nothing without an issue in context', () => {
        expect(IssueActionCommands.build({ idProject: 1, issue: null }, data, () => {}, t)).toEqual(
            []
        );
    });
    it('sets state via patch', () => {
        const patch = vi.fn();
        IssueActionCommands.build({ idProject: 1, issue }, data, patch, t)
    ...
    it('assigns a member and offers assign-to-me', () => {
        const patch = vi.fn();
        const cmds = IssueActionCommands.build({ idProject: 1, issue }, data, patch, t);
    ...
    it('interpolates the data name into the translated title frame', () => {
        const cmd = IssueActionCommands.build({ idProject: 1, issue }, data, () => {}, t).find(
    ...
    it('omits assign-to-me when there is no current user id', () => {
        const cmds = IssueActionCommands.build(
            { idProject: 1, issue },
...
describe('IssueActionCommands.buildClone', () => {
    it('offers a clone command on an open issue when the user can create', () => {
        const clone = vi.fn();
        const cmd = IssueActionCommands.buildClone({ idProject: 1, issue }, true, clone, t)!;
    ...
    it('returns null without an open issue or create permission', () => {
        expect(
            IssueActionCommands.buildClone({ idProject: 1, issue: null }, true, () => {}, t)
        ).toBeNull();
        expect(
            IssueActionCommands.buildClone({ idProject: 1, issue }, false, () => {}, t)
        ).toBeNull();
    });
});
